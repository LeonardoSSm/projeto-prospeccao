...
| ADR-016 | Receita Federal (CNPJ) como fonte primária de descoberta, antes de Google Places | Aceita | Dado aberto e gratuito, sem chave de API nem cobrança por evento (`docs/ANALISE_FONTES_DADOS_PROSPECCAO_LOCAL.md`). Implementado como mais um `PlacesProvider` (`ReceitaFederalPlacesProvider`) atrás da mesma interface do mock — Campaigns/Discovery/Leads não sabem a diferença. Importação é um script offline (`apps/core-api/scripts/cnpj-import/`), nunca uma chamada ao vivo: os arquivos da Receita são dumps mensais (~6-7GB, Brasil inteiro por arquivo), não uma API de busca. Validado com dado real: Fortaleza e 10 cidades do Ceará importadas (71.972 estabelecimentos, 10 nichos), campanhas reais trouxeram leads de verdade |
| ADR-017 | Geocodificação assíncrona via Nominatim (OpenStreetMap) para leads sem coordenada | Aceita | CNPJ traz endereço em texto, nunca lat/long. `NominatimGeocodingService` (`src/discovery/geocoding/`) enfileira e geocodifica em processo, respeitando o limite público de 1 req/s do Nominatim — nunca bloqueia `LeadsService.upsertFromSource`, é best-effort (sem persistência entre restarts do core-api). Só dispara para leads sem `latitude`/`longitude` (o mock já gera coordenada fake, então não é afetado). Validado com dado real: campanha para Sobral/CE geocodificou os 5 leads reais com coordenadas corretas |
| ADR-018 | Extração real de WhatsApp/Instagram do próprio site do lead, via crawler já existente | Aceita | O audit-worker já visita o site (Playwright, para o Lighthouse) — antes só detectava presença (`hasWhatsAppCta` booleano). `contact-extraction.ts` extrai o valor real do href (`wa.me/`, `api.whatsapp.com`, `instagram.com/`), fora do sandbox do navegador. `AuditingService.applyCompleted` cria `LeadContact` (WHATSAPP/INSTAGRAM) a partir disso via `LeadsService.attachDiscoveredContact` — best-effort, fora da transação principal, deduplicado (reprocessar o mesmo resultado não duplica contato). Validado com payload real publicado direto na fila `prospector.audit.completed.v1` contra um lead real |
| ADR-019 | Central de Dados: motor de CRUD genérico via introspecção do Prisma, em vez de 25 telas bespoke | Aceita | O frontend só cobria 4 das 25 tabelas (Leads/Campanhas/Nichos + drill-down por lead) — sem visão de catálogo CNPJ, jobs, propostas/outreach/auditorias fora do contexto de um lead, log de segurança etc. `AdminService` (`apps/core-api/src/admin/`) lê `Prisma.dmmf.datamodel.models` em runtime e expõe list/get/update/delete genéricos via `this.prisma[modelKey]`, evitando 25 controllers quase idênticos. `admin-registry.ts` declara, por tabela, o filtro de tenant — a maioria tem `organizationId` direto, mas várias só chegam lá por cadeia de relação até Lead/Campaign (ex.: `AuditFinding` → `audit.snapshot.lead.organizationId`); sem isso um ADMIN de um tenant veria dado de outro através desta tela, a única "porta" com acesso genérico ao banco inteiro. `AuditLog` fica `readOnly` (editar/apagar uma trilha de auditoria de segurança destruiria a garantia que ela existe pra dar). Restrito a `membershipRole === "ADMIN"` (`AdminRoleGuard`), tanto no backend (garantia real) quanto no frontend (só UX — esconde o link "Central" pra quem não é admin). Validado end-to-end: listagem/busca/paginação, edição com proteção de campos imutáveis (id/organizationId/createdAt/updatedAt), bloqueio de escrita em tabela read-only, e captura amigável de violação de FK ao apagar um registro com dependentes |
